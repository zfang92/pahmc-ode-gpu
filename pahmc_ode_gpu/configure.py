...
class Configure:
    ...
    def check_all(self):
        ...
        assert (type(self.obsdim) == list \
                ...
                and len(self.obsdim) >= 1 \
                and self.obsdim == list(np.floor(self.obsdim)) \
                and self.obsdim == list(set(self.obsdim)) \
                and 1 <= min(self.obsdim) and max(self.obsdim) <= self.D), \
            "'obsdim' must be a non-empty, one-dimensional list that" \
            + " contains only plain integers between 1 and D (inclusive);" \
    ...
    def get_stimuli(self):
        ...
        if filename.exists():
            ...
            print('\nNo external stimuli provided; set to zero.')
